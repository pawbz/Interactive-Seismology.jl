...
# ╔═╡ 03b283dc-10f5-432d-9a63-a9a327a4d545
begin
    # Field array allocations
    vy = zeros(nz, nx)
    dvydx = zeros(nz, nx)
    dvydz = zeros(nz, nx)
    σyx = zeros(nz, nx)
...
```
*The derivative property of Fourier Transform*: first order differentiation of a function along the $x$ dimension equivalent to multiplying its Fourier Transform by $\imath k_x$ in the wavenumber domain.\
...
begin
	...
	vy .= [exp(-((x - 0.0)^2 /11) - ((z - 0.0)^2 /1)) for z in zgrid, x in xgrid];
	vy .= Dz(vy)
	vy0 = copy(vy) # save initial conditions as vy shall be updated iteratively 
	nothing
end
...
# ╔═╡ e915ec11-be3f-4a9a-b9fb-91ee2c1c3b13
ntsteps = Int(floor(r/(mean(sqrt.(μ./ρ))*dt)))

# ╔═╡ 013321df-8665-4254-828e-37cfed193d8d
snaps_store = propagate(ntsteps)

# ╔═╡ b88aed3d-9cb0-4377-8797-65385ab59436
@bind tsnap Slider(1:length(snaps_store), default=length(snaps_store))

# ╔═╡ da1d4263-f5c6-49bf-b3a7-3e75f763acd8
# ╠═╡ disabled = true
#=╠═╡
md"""
## Assignment II - Deadline 26th August 2022 5PM
1) Imagine you would like to study mantle heterogeneities using SH waves. Adjust the spatial and temporal scales of the notebook with realistic values, proper physical units, and re-run the simulation. Choose shear modulus and density values accordingly. Don't forget the satisfy the Courant's criteria, otherwise, simulations will be unstable.
 
2) Run simulations for two different cases: (A) values of the shear modulus from the upper mantle and (B) another from the lower mantle. Explain the differences that you see in the simulations. Compute the shear-wave velocity $\sqrt{\frac{\mu}{\rho}}$ in both cases. 
 
3) Place two receivers in the medium and record $v_y$ at both of them and plot their time series for both cases (A) and (B). Comment on the differences.

4) Related to the second question, in which of the cases, do the receivers record high amplitudes? Reason out.

5) Now calculate the time of energy onset i.e., the travel time between the source at $(0,0)$, at both the receivers. Let's denote them using $t_A$ and $t_B$ and comment on their differences.   

6) Related to the previous question, compute $c=d/|t_A - t_B|$ where $d$ is the distance between the receivers. What does $c$ give you? How is it related to the shear-wave velocity of the medium? Comment on whether $c$ changes with the receiver locations. How would you use $c$ to characterize the medium?

#### Optional

7) At each time step, calculate the total momentum of the system and plot it as a function of the time step. What do you observe? Explain.
 
8) Run the simulations for different values of the Courant number, observe differences in the total momentum.

9) Modify the notebook to simulate waves in a two-layer medium with parameters $\mu_1$, $\mu_2$, $\rho_1$ and $\rho_2$. Observe scattering of waves. 
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "8ee2101f6033ad34322791713028e5d516685d3a"
