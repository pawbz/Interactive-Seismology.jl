...
function plot_planewave(u, t1=0)
    ...
    clim = (-maximum(abs.(up0)), maximum(abs.(up0)))
        ...
        up = [real(fn_u(x, z, tplot)) for z in zgrid, x in xgrid]
	fig=Plot(Layout(width=325, height=300))
        add_trace!(fig, heatmap(x=xgrid, y=zgrid, z=up, title="", yflip=true, colorscale=:seismic, zmin=clim[1], zmax=clim[2]))
	plot(fig)
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "ef4f58411449c11eb8ddf88ec8077915f567acc6"
