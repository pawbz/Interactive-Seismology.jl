...
frequency (Hz): $(@bind fp Slider(range(1, stop=5, length=10), default=2, show_value=true))
"""
...
# assume a time grid
tgrid = range(-100, stop=100, length=2048)
...
plot(tgrid, ureceiver, w=2, xlim=(-50, 100), label="recorded pulse at $(xr) km", ylim=(0, 0.2e-1), xlabel="time (s)", ylabel="amplitude")
...
function plot_planewave(u, t1=0)
    ...
    # substitute imaginary unit, ω
    fn_u = build_function(subs(u, fp / 2 / pi), x, z, t, expression=Val{false})

    up0 = [real(fn_u(x, z, 0)) for z in zgrid, x in xgrid]
    clim = (-maximum(abs.(up0)), maximum(abs.(up0)))

    gif(@animate(for t in 1:10
        up = [real(fn_u(x, z, t)) for z in zgrid, x in xgrid]
        heatmap(xgrid, zgrid, up, title="", yflip=true, c=:seismic, clim=clim, colorbar=nothing, frame=nothing, axis=nothing, size=(300, 300))
    end),)
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "6e8d0889b0125c8bd6ffacc5784e233af09f0167"
...
git-tree-sha1 = "0fdf00c111dd855981ccb7083d82c3cc0e06d973"
