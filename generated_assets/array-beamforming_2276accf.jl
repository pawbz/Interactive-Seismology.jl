...
begin
    ...
    #= Legacy Cartesian-only renderer retained in version control history.
...
begin
    function Base.show(io::IO, ::MIME"text/html", w::ArrayBeamformingInput)
        ...
        const ids={'bf-period':'period','bf-velocity':'velocity','bf-azimuth':'azimuth'},LIMIT=20;let result=null,view='polar';
        ...
        function emit(){par.value={...state,receivers:state.receivers.map(q=>[q[0],q[1]])};par.dispatchEvent(new CustomEvent('input'))}
        ...
        par.addEventListener('input',onControl,true);par.addEventListener('change',onControl,true);by('bf-array').addEventListener('click',arrayClick);by('bf-polar').onclick=()=>setView('polar');by('bf-cartesian').onclick=()=>setView('cartesian');by('bf-reset').onclick=()=>{state.receivers=[[-8,-3.5],[-5,6],[-1,-7],[0,0],[3.5,7],[5.5,-4],[8,2],[1.5,3]];emit()};by('bf-clear').onclick=()=>{state.receivers=[];emit()};window.addEventListener('beamforming-results',event=>{result=event.detail?JSON.parse(event.detail):null;draw()});new ResizeObserver(draw).observe(par);labels();draw();
        </script></div>
        """)
    end
