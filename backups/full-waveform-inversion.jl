...
# ╔═╡ f4003583-637a-4a90-a8b5-dd94adfbf7e2
using Random, Distributions

# ╔═╡ c3cdb358-9a3d-4fd9-bc49-b9078aea5c77
...
# ╔═╡ dd855939-0422-4e2d-b6db-df15436608fe
md"""
...
# ╔═╡ 9173244e-50d8-4fa6-bf44-41bcf06c67c2
χ(m, m0) = log(m / m0)

# ╔═╡ 92cd5081-a27b-406f-b18d-9c7773aef08d
χ⁻¹(M, m0) = exp(M) * m0

# ╔═╡ be4ca7b8-fbfa-4879-86cd-722628ddc4ca
χ⁻¹(χ(0.5, 3), 3)

# ╔═╡ 0fd785a8-33b3-4790-8e40-4f63385bcd61
begin
	cov = CovarianceFunction(2, Matern(0.005, 3/4))
	pts = range(0, stop=1, length=512)
	grf = GaussianRandomField(cov, CirculantEmbedding(), pts, pts, minpadding=113)
	heatmap(grf)
end

# ╔═╡ 2d804672-1745-4a91-a06e-282a498ecab2
GaussianRandomFields.sample(grf)

# ╔═╡ 8b23ea2c-264c-4295-b4a6-1ec4c1a076e9
sample(m, σ, bounds) = rand(Distributions.Truncated(Distributions.Normal(m, σ), bounds[1], bounds[2]))

# ╔═╡ c2f6cece-27d2-4ad2-a844-96d52fc67b7a
"""
PML Extend a model on all four sides
"""
function rand_boundary(mod::Array{Float64,2}, bounds, np=50)
	m0 = mean(mod)
	modex = χ⁻¹.(GaussianRandomFields.sample(grf) .* 0.1 .+ χ(mean(mod), m0), m0)
	# modex = deepcopy(mod)
	nz, nx = size(mod)
	for ix in np+1:nx-np
		for iz in np+1:nz-np
			modex[iz, ix] = mod[iz, ix]
		end
	end
    # modex = zeros(eltype(mod), size(mod, 1) + 2 * np, size(mod, 2) + 2 * np)
 ...
 #    m0 = mean(mod)
 #    χbounds = χ.(bounds, m0)
 #    σ = reverse(range(0.01, stop=1, length=np))
 #    for ix in 1:nx
 #        for (iz, s) in zip(1:np, σ)
 #            m = χ(mod[iz, ix], m0)
 #            modex[iz, ix] = χ⁻¹(sample(m, s, χbounds), m0)
	# 		m = χ(mod[nz-iz+1, ix], m0)
 #            modex[nz-iz+1, ix] = χ⁻¹(sample(m, s, χbounds), m0)
 #        end
	# end
	# for iz in 1:nz
 #        for (ix, s) in zip(1:np, σ)
 #            m = χ(mod[iz, ix], m0)
 #            modex[iz, ix] = χ⁻¹(sample(m, s, χbounds), m0)
	# 		m = χ(mod[iz, nx-ix+1], m0)
 #            modex[iz, nx-ix+1] = χ⁻¹(sample(m, s, χbounds), m0)
 #        end
	# end
...
begin
	...
	# μ = rand_boundary(μ, [0.5, 3], 100)
	
    # lets add density perturbation
    ρ[1:256, :] .= 2.0
	ρ = rand_boundary(ρ, [0.5, 3], 100)
    nothing
end

# ╔═╡ dcbc8f5b-a39d-4ef9-94c8-2a4d47cdd2a5
heatmap(ρ)

# ╔═╡ df07ad66-e095-4ce9-8b33-edeb112925c1
heatmap(ρ, aspect_ratio=1)
...
# ╔═╡ bfa46f63-cd1e-4054-8e57-084f5d477ec5
md"## Forward Simulation"
...
function ricker(fqdom::Real,
    tgrid::StepRangeLen;
    tpeak::Real=tgrid[1] + 1.5 / fqdom, # using approximate half width of ricker
...
# ╔═╡ 6ffbe620-f386-4020-9292-717401d56d47
f = ricker(1, tgrid)
...
function propagate!(fields, pa)
    ...
    @progress for it = 1:nt
        ...
        @. vy = vy + inv(ρ) * (dσyxdx + dσyzdz + f) * dt


		# loop over source locations
		
		vy[256, 256] =+ inv(ρ[256, 256]) * f[it] * dt
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "4ce0cc78fc8fe099f2472b8e520c04d5f986b603"
...
deps = ["Arpack", "FFTW", "FastGaussQuadrature", "LinearAlgebra", "RecipesBase", "SpecialFunctions", "Statistics"]
git-tree-sha1 = "d9c335f2c06424029b2addf9abf602e0feb2f53e"
