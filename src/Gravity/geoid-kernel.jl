...
function modes(η, ℓ)
    ...
    hom = map(p -> (p, 1.0, 0.0), (ℓ - 1, -ℓ - 2))
    # particular modes driven by P = r^s, s = ℓ and s = -(ℓ+1); take cP = 1
    particular = map((ℓ, -(ℓ + 1))) do s
        p = s + 1
        denom = p * (p - 1) + 2p - (2 + L) + 2 * (p + 2)
        (p, ((p - 1) / η) / denom, 1.0)
    end
    # a fixed-length NTuple (not a growable Vector) so `Ymat`'s `hcat(...)` splat below stays
    # type-stable -- splatting a runtime-length collection is a classic inference killer, and
    # this instability was previously cascading all the way up through `propagator`,
    # `layer_propagator`, and `geoid_response`, making its own return type ~90% `Any`.
    return (hom..., particular...)
end
...
function Ymat(r, η, ℓ)
    x = r / A_EARTH
    # `map` over `modes`' fixed-length NTuple keeps the result an NTuple too (a comprehension
    # would silently widen it back to a Vector), which is what keeps this `hcat` splat
    # type-stable -- see the note in `modes` above.
    cols = map(modes(η, ℓ)) do (p, cU, cP)
        mode_state(x, η, ℓ, p, cU, cP)
    end
    hcat(cols...)
end
