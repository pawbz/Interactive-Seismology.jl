...
struct CrossCorrelationWorkspace{T, PX, PY, PI}
    xspec::Vector{Complex{T}}
    yspec::Vector{Complex{T}}
    forward_x::PX
    forward_y::PY
    inverse_y::PI
end

function CrossCorrelationWorkspace(::Type{T}, n::Integer) where {T<:AbstractFloat}
    xspec = zeros(Complex{T}, n)
    yspec = zeros(Complex{T}, n)
    CrossCorrelationWorkspace(
        xspec,
        yspec,
        plan_fft!(xspec; flags=FFTW.ESTIMATE),
        plan_fft!(yspec; flags=FFTW.ESTIMATE),
        plan_ifft!(yspec; flags=FFTW.ESTIMATE),
    )
end

"""
    xcorr!(out, x, y, workspace)

Write the full, zero-padded cross-correlation of real traces `x` and `y` into `out`.
`workspace` owns the FFT buffers and plans, so a source stack can reuse them without
allocating a padded trace and FFT result for every source.
"""
function xcorr!(out::AbstractVector{T}, x::AbstractVector{T}, y::AbstractVector{T}, workspace::CrossCorrelationWorkspace{T}) where {T<:AbstractFloat}
    xspec = workspace.xspec
    yspec = workspace.yspec
    fill!(xspec, 0)
    fill!(yspec, 0)
    @views xspec[1:length(x)] .= x
    @views yspec[1:length(y)] .= y

    mul!(xspec, workspace.forward_x, xspec)
    mul!(yspec, workspace.forward_y, yspec)
    @. yspec = conj(xspec) * yspec
    mul!(yspec, workspace.inverse_y, yspec)

    # `fftshift` is a right circular shift by `length(out) ÷ 2`.
    shift = length(out) ÷ 2
    @inbounds for i in eachindex(out)
        source_i = i - shift
        source_i <= 0 && (source_i += length(out))
        out[i] = real(yspec[source_i])
    end
    return out
end
...
function cross_correlations!(cross, seis1, seis2)
    workspace = CrossCorrelationWorkspace(eltype(cross), size(cross, 1))
    @views for i in axes(seis1, 2)
        xcorr!(cross[:, i], seis1[:, i], seis2[:, i], workspace)
    end
    return cross
end
...
# ╔═╡ 280e1b7d-7d65-4f03-81d4-36e5e59495af
"""
    selected_cross_correlation_stack(rec1, rec2, srcloc, wavelet, nt, dt, velocity,
                                     contributions, threshold)

Compute the selected average cross-correlation directly, without retaining one
seismogram pair and correlation trace per source. The selection and accumulation order
match [`average_selected_cross_correlations`](@ref), so this is a storage optimization
for diagnostics rather than a change to the recovered response.
"""
function selected_cross_correlation_stack(rec1, rec2, srcloc, wavelet, nt, dt, velocity, contributions, threshold)
    nsrc = length(srcloc)
    nlag = 2nt - 1
    avg = zeros(Float32, nlag)
    nsrc == 0 && return avg

    use_threshold = threshold > 0 && length(contributions) == nsrc
    seis1 = zeros(Float32, nt)
    seis2 = zeros(Float32, nt)
    corr = zeros(Float32, nlag)
    workspace = CrossCorrelationWorkspace(Float32, nlag)
    nselected = 0

    @inbounds for j in eachindex(srcloc)
        if !use_threshold || contributions[j] > threshold
            delaywav!(seis1, wavelet, get_traveltime(rec1, srcloc[j], velocity), dt)
            delaywav!(seis2, wavelet, get_traveltime(rec2, srcloc[j], velocity), dt)
            xcorr!(corr, seis1, seis2, workspace)
            nselected += 1
...
begin
    ...
    _gren_ab = selected_cross_correlation_stack(rec1, rec_origin, srcloc_for_noise, _three_station_wavelet, nt, dt, vel, contributions, _subsamp_threshold)
    _phase_ab = phase_correct_trace(_gren_ab, _true_ab, tgrid_xcorr, d_ab, vel, f0)
    _trace_ab = apply_phase_correction ? _phase_ab.trace : _gren_ab
    _measurements_ab = branch_velocity_measurements(_trace_ab, tgrid_xcorr, d_ab, vel, f0, phase_branch)

    _gren_bc = selected_cross_correlation_stack(rec_origin, rec2, srcloc_for_noise, _three_station_wavelet, nt, dt, vel, contributions, _subsamp_threshold)
    _phase_bc = phase_correct_trace(_gren_bc, _true_bc, tgrid_xcorr, d_bc, vel, f0)
    _trace_bc = apply_phase_correction ? _phase_bc.trace : _gren_bc
