...
function okada_surface_field(flt; n=56, world_half=90.0, wavelength=0.056)
    # `flt` is the widget's raw Dict{String,Any} bond value -- pulled out to concrete Float64/Bool
    # locals ONCE here rather than re-looked-up (Any-typed, dynamically dispatched) at every one of
    # the n×n grid points below.
    E0, N0 = Float64(flt["E0"]), Float64(flt["N0"])
    depth, strikeDeg, dipDeg = Float64(flt["depth"]), Float64(flt["strikeDeg"]), Float64(flt["dipDeg"])
    L, W, sStrike, sDip = Float64(flt["L"]), Float64(flt["W"]), Float64(flt["sStrike"]), Float64(flt["sDip"])
    ascending = Bool(flt["ascending"])
    ...
    for (j, N) in enumerate(ns), (i, E) in enumerate(es)
        ...
        d = okada_surface_displacement(Erel, Nrel;
            depth=depth, strike_deg=strikeDeg, dip_deg=dipDeg,
            L=L, W=W, U1=sStrike, U2=sDip)
        uE[j, i], uN[j, i], uZ[j, i] = d.uE, d.uN, d.uZ
    end
    incidence = deg2rad(23.0)
    lookE = (ascending ? 1 : -1) * sin(incidence)
    lookU = cos(incidence)
