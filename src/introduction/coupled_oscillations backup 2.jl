...
fma2 = map(f -> begin
        simplify(expand_derivatives(substitute(f, subs_ansatz)) / cos(ω*t))
    end, fma)

# ╔═╡ e44ae9ed-394e-4aed-ac41-a638470fef42
md"""
...
# Build stiffness matrix K and mass matrix M for N masses with fixed ends
k_val = 1.0   # spring constant (can be exposed as a slider)
...
# ╔═╡ 49b86489-1caf-42f8-bbc4-3045945e14c0
# Solve the generalized eigenproblem K v = λ M v
eigsol = eigen(Symmetric(K), Hermitian(M))

# ╔═╡ 74222724-1598-41aa-a627-9c50c7d3a4e0
λ = real(eigsol.values)

# ╔═╡ 432ca8ad-cd28-44b2-bdc1-02c0ff3941a0
perm = sortperm(λ)

# ╔═╡ b25fd74f-4ba1-4d2f-aa74-5d498526eca7
λs = λ[perm]

# ╔═╡ 0b5bb8c9-f2fb-47b3-ba46-9440d7d550b2
vecs = eigsol.vectors[:, perm]

# ╔═╡ 08703fac-6e06-4a8e-95f9-5aa6d48789f8
ωs = sqrt.(λs)
...
# ╔═╡ 8d47e725-7be2-4b0e-a99a-50558cbf6147
theme(:dark)
...
function spring_between(x1, x2; ncoils=8, amp=0.08, points_per_coil=20)
    ...
    xs = Float64[]
    ys = Float64[]
    total_pts = max(2, Int(ncoils * points_per_coil))
    for i in 0:total_pts
        s = i / total_pts
        x = x1 + s * L
        # sinusoidal shape along the spring
        y = amp * sin(2pi * ncoils * s)
        push!(xs, x)
        push!(ys, y)
    end
    return xs, ys
...
function plotmasses(a, t=""; wide=false)
    xs = first.(a)
    ys = last.(a)
    # choose size and margins depending on wide flag
    sz = wide ? (1100, 320) : (700, 240)
    lmar = wide ? 80 : 40
    rmar = wide ? 80 : 40
    tmar = wide ? 40 : 20
    bmar = wide ? 40 : 20
    ...
    p = scatter(xs, ys, marker = (:rect, 14), color = :yellow, markerstrokecolor = :white,
                markerstrokewidth = 1.5, xlim = (-3,3), ylim = (-2,2), yaxis = nothing,
                xlabel = L"x", label = nothing, size = sz, title = t,
                left_margin = lmar, right_margin = rmar, top_margin = tmar, bottom_margin = bmar)
...
# ╔═╡ 0133e2a2-525a-4ef2-a852-487c9191eba9
plotmasses(points, "N=$N particles; Mode=$imode; t=$tplot", wide=true)
...
manifest_format = "2.0"
project_hash = "b1e14138cb6dd96bea10d2f250b206364878050b"
