...
md"""
## Steps 1 & 2: Pick an Earthquake, Then a Station
...
# ╔═╡ 6a1e0001-0000-4000-8000-000000000012b
begin
...
begin
    ...
    One widget, two linked flat-map panels. **Left**: drag a box to set the earthquake search
    region, set a minimum magnitude, press **Search** (queries USGS), then click the nearest
    dot to select an earthquake -- dot size scales with magnitude. **Right**: once an
    earthquake is selected, drag a box and press **Search** to find GSN (`IU`) stations that
    were actually operating during that event (not just nearby ones), then click one to
    select it; the epicenter is drawn as a star and the great-circle path + distance appear
    once a station is picked. Both panels' fetches are gated behind their own explicit
    **Search** button -- dragging a box or adjusting the magnitude slider never fires a
    network call on its own. Fully self-contained: the fetched catalogs, the epicenter, and
    the enabled/disabled state of the station panel are all tracked as plain JS state within
    this one widget instance: Julia only ever sees four dispatched actions (`eq-search`,
    `eq-select`, `st-search`, `st-select`) through the bound value.
    """
...
<div id="sexwidget" style="display:flex;flex-direction:column;align-items:center;width:100%;color:#9ca3af">
  ...
  <div class="sex-title">
    <div class="sex-title-desc">Every seismogram starts with two choices: which earthquake, and which station recorded it.</div>
    <div class="sex-title-hint">drag a box &middot; Search &middot; click a dot &middot; station panel unlocks once an earthquake is picked</div>
  </div>
  ...
  <div class="sex-controls">
    ...
    <div class="sex-control-group" style="flex:1 1 220px">
      ...
      <div class="sex-readout"><span style="color:#ef4444">&#9679;</span> earthquake &middot; <span style="color:#3b82f6">&#9650;</span> station &middot; <span style="color:#facc15">&#9679;/&#9733;</span> selected/epicenter &middot; <span style="color:#22d3ee">line</span> = great-circle path</div>
    </div>
  </div>
</div>
<script>
  ...
  function hidpi(cv, cx){ cv.width=Math.round(W*DPR); cv.height=Math.round(H*DPR); cv.style.width=W+'px'; cv.style.height=H+'px'; cx.setTransform(DPR,0,0,DPR,0,0) }
  ...
  hidpi(eqCvs, eqCtx); hidpi(stCvs, stCtx)
  ...
  function drawEqEvents(){
    ...
    for(let i=0;i<eqData.length;i++){
      ...
      const [x,y] = lonlatToXY(lon,lat)
      const r = Math.max(2.5, Math.min(11, 1.6+mag*1.2))
      eqCtx.beginPath(); eqCtx.arc(x,y,r,0,2*Math.PI)
      eqCtx.fillStyle = i===eqSelectedIdx ? '#facc15' : 'rgba(239,68,68,0.75)'
      eqCtx.fill()
  ...
  par.querySelector('#sex-minmag').addEventListener('input', e=>{
...
"""
Every dispatch from the widget that follows an earthquake selection (`st-search`, `st-select`)
also echoes back that earthquake's own fields (`eqlat`, `eqlon`, ...) alongside its own --
this cell reads whichever of the two shapes `sex` currently holds. Pluto cells can't be
self-referential (there's no way to read this variable's *previous* value from inside its own
definition), so the widget re-sending the earthquake fields on every action is what lets this
stay a plain, stateless, input-only computation instead of needing a mutable `Ref`.
"""
...
elseif sex isa AbstractDict && sex["action"] in ("st-search", "st-select") && haskey(sex, "eqtime")
...
manifest_format = "2.0"
project_hash = "cf5c2a4d73de915934a0b68b7a81b723b0b71768"
