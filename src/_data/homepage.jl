...
A [collection](https://pawbz.github.io/Interactive-Seismology.jl/) of *reactive notebooks* using the [Julia](https://julialang.org/) notebook system, Pluto. These notebooks are designed to illustrate fundamental concepts in seismology in an interactive manner. They are primarily intended to support our course on Seismology (ES218), and are accessible to students with minimal programming experience. The notebooks contain all the essential mathematical expressions, which are easily manipulated using [Symbolics.jl](https://symbolics.juliasymbolics.org/dev/), a modern Computer Algebra System (CAS). By using these standalone notebooks, students can intuitively explore and gain a better understanding of wave phenomena in seismology.
    ...
    "highlights" => [
        ...
        Dict("name" => "Ray Tomography",
         ...
         Dict("name" => "Surface Wave Dispersion",
             "text" => md"""
             Surface waves, such as Rayleigh and Love waves, exhibit dispersion, meaning their velocity depends on frequency.
                **These interactive notebooks allow students to modify layer thickness, velocities, and densities in real-time and immediately observe how these structural changes influence the shape and characteristics of surface wave dispersion curves.**
             """,
             "img" => "$(root_url)/assets/movies/Love-dispersion.gif"
        ),
        Dict("name" => "Global Seismic Phases",
             "text" => md"""
             Seismic waves generated by earthquakes travel through the Earth following complex paths, creating various phases that arrive at different times.
             **These interactive visualizations allow students to explore how seismic waves propagate through Earth's layered structure, observe ray paths for different phases like P, S, PKP, and SKS waves, and understand how source depth and receiver distance affect arrival times and wave trajectories.**
             """,
             "img" => "$(root_url)/assets/movies/global_seismic_phases.gif"
        )
    ]
)