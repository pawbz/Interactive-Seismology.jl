...
The acceptance rate of the Metropolis criterion has to be around $30-50\%$. If the acceptance rate is, we are not moving fast enough in the model space. On the other hand, if the acceptance rate is much smaller, then we are wasting computer resources as most of the samples are not accepted.
"""]))
...
The function returns two objects: `msamples`, which is an array of the generated samples, and `acceptance_history`, which is an array that keeps track of whether a sample was accepted or rejected at each step of the Metropolis-Hastings algorithm.
"""
...
function generate_samples(m0, walk, L; N=10)
    ...
    while true
        ...
        (i ≥ N) && (break) # if already tried N times


    end
    return msamples, acceptance_history
...
md"""
###### Use Metropolis criterion? $(@bind metro Select([L2Dexample=>"Yes", nothing=>"No"]))
"""

# ╔═╡ c75c0510-fbca-48db-94b9-2c1f62096a2d
range_example = range(-10, stop=10, length=256)

# ╔═╡ 02ef9d57-a983-4756-9171-494f622f3db9
L2Dexample_grid = [L2Dexample([x, y]) for x in range_example, y in range_example];
...
heatmap(range_example, range_example, L2Dexample_grid, c=:grays, title="Likelihood Distribution", aspect_ratio=1)

# ╔═╡ 48a5055f-a239-4d8e-9369-74e1fc9a9bd9
md"## Gravity Inversion Example"
...
function sampler_input_2Dexample()

    return PlutoUI.combine() do Child
        p = [md"""
Initial model m₁ = $(Child("m₁0", Slider(range_example, default=-5, show_value=true))) and m₂ =
 $(Child("m₂0", Slider(range_example, default=-5, show_value=true)))
             """,
        ]
        σ = [md"""
...
@bind ex1 sampler_input_2Dexample()

# ╔═╡ f9c4bfe0-6fd6-4e72-a3f1-76757957373f
function randomwalk_2Dexample_independent(m)
    sigma = ex1[:σglobal]
    return sigma .* (randn(2) .- 0.5)
end

# ╔═╡ 6d2b530a-0376-47a0-9e15-c22bae6cac32
function randomwalk_2Dexample_local(m)
    sigma = ex1[:σlocal]
...
md"""
Generate $(@bind Nex1 Slider(1:256, default=20, show_value=true))
$(@bind walk Select([randomwalk_2Dexample_independent=>"Independent Samples", randomwalk_2Dexample_local=>"Markov Chain"]))
"""

# ╔═╡ 9d52551f-1384-487b-9a81-ddd42a98639f
msamples, acceptance = generate_samples([ex1[:m₁0], ex1[:m₂0]], walk, metro, N=Nex1)

# ╔═╡ aa1c78b9-2cdb-49c8-8e5f-a34ff1a9c059
begin
    p1 = heatmap(range_example, range_example, L2Dexample_grid, c=:grays, lim=(-10, 10), xlabel="m₁", aspect_ratio=1, ylabel="m₂", title="Accepted 100/$(length(acceptance)) with ratio $(round(count(acceptance)/length(acceptance), digits=3))")
    @gif for isamp in 2:length(msamples)
        # println(m, acc)
        plot!(p1, [msamples[isamp-1][2], msamples[isamp][2]], [msamples[isamp-1][1], msamples[isamp][1]], c=:blue, label=nothing)
    end
    scatter!(p1, [msamples[1][1:1]], [msamples[1][2:2]], c=:yellow, m=:x, label="Initial Sample")
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "2e24848f0e9497b998b63c1998f74101c6dbfb78"
